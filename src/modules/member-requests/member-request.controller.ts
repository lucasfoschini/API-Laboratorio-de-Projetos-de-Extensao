...
export class MemberRequestController {
  async create(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      res.status(201).json(await svc.create(req.params.id, req.user.id, req.body.message));
    } catch (e) { next(e); }
  }
  async listByProject(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      if (!req.user) { res.status(401).json({ message: "Unauthorized" }); return; }
      res.json(await svc.listByProject(req.params.id, req.user.id));
    } catch (e) { next(e); }
  }
  async review(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      if (!req.user) { res.status(401).json({ message: "Unauthorized" }); return; }
      res.json(await svc.review(req.params.requestId, req.user.id, req.body.status as MemberRequestStatus));
    } catch (e) { next(e); }
  }
